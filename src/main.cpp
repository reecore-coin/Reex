...
// Copyright (c) 2017-2020 The Reecore developers
...
/** Fees smaller than this (in ureex) are considered zero fee (for relaying and mining)
...
// Reject block.nVersion=4 blocks when 95% (75% on testnet) of the network has upgraded:
    if(block.nVersion < 5 && CBlockIndex::IsSuperMajority(5, pindexPrev, Params().RejectBlockOutdatedMajority())) {
        return state.Invalid(error("%s : rejected nVersion= block", __func__),
            REJECT_OBSOLETE, "bad-version");
    }
    return true;
}
...
int ActiveProtocol()
{
    ...
    if (IsSporkActive(SPORK_15_PROTOCOL_ENFORCEMENT_2))
        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT_2;

    if (IsSporkActive(SPORK_14_PROTOCOL_ENFORCEMENT_1))
        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT_1;
